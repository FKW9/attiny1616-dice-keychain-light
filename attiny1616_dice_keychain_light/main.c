...
void disable_test_pins(void);
uint8_t check_test_pins(void);
...
ISR(TCA0_CMP0_vect)
{
	...
	static uint16_t t_test		        = 0;
	static uint16_t t_test_reset        = 0;
...
ISR(TCB0_INT_vect)
{
	if (current_dice_state == 2 || current_dice_state == 3 || current_dice_state == 4 || current_dice_state == 5 || current_dice_state == 99)
	{
		if (blink_flag == 0)
		{
			dice_timer_routine();
...
void enable_rng_adc_channels(void){
	/* PORT setting on TP4 */
	TP4_set_level(false);
	TP4_set_dir(PORT_DIR_IN);
	TP4_set_pull_mode(PORT_PULL_OFF);
	TP4_set_isc(PORT_ISC_INTDISABLE_gc);
...
void disable_rng_adc_channels(void){
	...
	TP4_set_level(false);
	TP4_set_dir(PORT_DIR_IN);
	TP4_set_pull_mode(PORT_PULL_UP);
	TP4_set_isc(PORT_ISC_INPUT_DISABLE_gc);
...
uint8_t check_test_pins(void){
	if(TP4_get_level() == 0 && TP5_get_level() == 0){
		set_mode(MODE_TEST);
		current_dice_state = 99;
		return 1;
	}
	return 0;
}
...
void disable_test_pins(void){
	...
	TP4_set_dir(PORT_DIR_IN);
	TP4_set_pull_mode(PORT_PULL_UP);
	TP4_set_inverted(false);
	TP4_set_isc(PORT_ISC_INPUT_DISABLE_gc);
		
	/* PORT setting on PC0 */
	TP5_set_level(false);
	TP5_set_dir(PORT_DIR_IN);
	TP5_set_pull_mode(PORT_PULL_UP);
	TP5_set_inverted(false);
	TP5_set_isc(PORT_ISC_INPUT_DISABLE_gc);
}